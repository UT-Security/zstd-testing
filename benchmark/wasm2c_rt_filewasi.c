  ...
  u32 pnum)
{
  ...
  for (u32 i = 0; i < iovcnt; i++) {
    ...
    if (nfd == WASM_STDOUT) {
      result = fwrite(WASI_MEMACCESS(wasi_data->instance_memory, ptr), 1, len, stdout);
    } else if (nfd == WASM_STDERR) {
      result = fwrite(WASI_MEMACCESS(wasi_data->instance_memory, ptr), 1, len, stderr);
    } else {
      result =
          POSIX_PREFIX(write)(nfd, WASI_MEMACCESS(wasi_data->instance_memory, ptr), len);
    }
...
u32 w2c_wasi__snapshot__preview1_fd_close(wasm_sandbox_wasi_data* wasi_data,
                                          u32 fd) {
  int nfd = get_native_fd(wasi_data, fd);
  if (nfd < 0) {
    return WASI_PERM_ERROR;
  }
  // For additional safety don't allow closing the input, output and error
  // streams
...
u32 w2c_wasi__snapshot__preview1_fd_seek(wasm_sandbox_wasi_data* wasi_data,
                                         u32 fd,
                                         u64 offset,
                                         u32 whence,
                                         u32 new_offset) {
  ...
  wasm_i64_store(wasi_data->instance_memory, new_offset, off);
  return 0;
}
