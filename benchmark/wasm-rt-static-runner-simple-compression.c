...
#include "simple_compression.wasm.h"
#include "uvwasi.h"
#include "uvwasi-rt.h"
...
int main(int argc, const char** argv) {
  ...
  w2c_simple__compression inst = { 0 };
  ...
  wasm2c_simple__compression_instantiate(&inst, &wasi_env);

  /* Call `inst`, using the mangled name. */
  w2c_simple__compression_0x5Fstart(&inst);

  /* Free the inst module. */
  wasm2c_simple__compression_free(&inst);
